...
const Task = ({ task }) => {
	...
	const [ DeleteTask ] = useMutation(DeleteTask_MUTATION);
	const [ ToggleTaskStatus ] = useMutation(ToggleComplete_MUTATION);

	// Format the date
	const createdAt = new Intl.DateTimeFormat('en-GB', { dateStyle: 'full' }).format(new Date(task.dateCreated));
	...
	return (
		<div className={ `card ${ task.complete && "complete" }` }>
			<div className={ task__wrapper }>
				<div role="button" className={ `task ${ task__title__date }` } onClick={ () => toggleTask(task) } onKeyDown={ () => toggleTask(task) } tabIndex={ task.slug }>
					<small className={ task__title }>{ task.title }</small>
					<small className={ task__date }><strong>Posted On:</strong> { createdAt }</small>
				</div>
